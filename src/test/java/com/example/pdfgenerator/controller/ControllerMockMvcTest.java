...
public class ControllerMockMvcTest {
    ...
    @Test
    public void generatePdf_whenValidInput_thenReturnPDFInResponse() throws Exception {
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        outputStream.assignBytes(new ObjectMapper().writeValueAsBytes(invoice));
        Mockito.when(pdfGeneratorService.getPdfDocumentByteArray(any(Invoice.class)))
                .thenReturn(new ByteArrayOutputStream().assignBytes(new ObjectMapper().writeValueAsBytes(invoice)).toByteArray());

        mockMvc.perform(MockMvcRequestBuilders.post("/api/v1/pdf")
